...
function fldCur(label, cur, slot, isTag) {
  const c = isTag ? `<span class="cur tag">${esc(cur)}</span>` : `<span class="cur">${esc(cur)}</span>`;
...
function fldProminent(label, cur, slot) {
  return `<span class="fld prominent"><span class="fl">${label}</span><span class="cur">${esc(cur)}</span><span class="arr">→</span><span class="slot ${slot}"></span></span>`;
}

/* ---------- gutters per category ---------- */
function gutter(cat) {
  if (cat === 'inbox') {
    return `<div class="gut">${cellHTML('→', 'Next')}${cellHTML('→', 'Deleg')}${deferHTML('Defer')}${cellHTML('✗', 'Drop')}</div>`;
  }
  if (cat === 'delegated') {
    return `<div class="gut">${cellHTML('✓', 'Done')}${cellHTML('↩', 'To me')}${deferHTML('Defer')}${cellHTML('✎', 'Edit')}</div>`;
  }
  if (cat === 'tickler') {
    return `<div class="gut">${cellHTML('→', 'Now')}${cellHTML('✓', 'Done')}${deferHTML('Re-defer')}${cellHTML('✎', 'Edit')}</div>`;
  }
  // next actions
  return `<div class="gut">${cellHTML('✓', 'Done')}${cellHTML('→', 'Deleg')}${deferHTML('Defer')}${cellHTML('✎', 'Edit')}</div>`;
}
...
function fields(cat, t) {
  if (cat === 'inbox') {
    return `<div class="fields">
      ${fldBlank('Priority', 'n')}${fldBlank('Due', 's')}${fldBlank('Project', 'm')}${fldBlank('To', 'to')}
    </div>`;
  }
  if (cat === 'delegated') {
    return `<div class="fields">
      ${fldProminent('To', t.to, 'to')}${fldCur('Priority', t.pri, 'n')}${fldCur('Due', t.due, 's')}${fldCur('Project', t.proj, 'm', true)}
    </div>`;
  }
  if (cat === 'tickler') {
    return ''; // no metadata
  ...
  // next actions
  return `<div class="fields">
    ${fldCur('Priority', t.pri, 'n')}${fldCur('Due', t.due, 's')}${fldCur('Project', t.proj, 'm', true)}${fldBlank('To', 'to')}
  </div>`;
}
...
function bandEl(title, sub, count) {
  ...
  b.className = 'band';
  b.innerHTML = `<span class="bt">${esc(title)}</span><span class="bs">${esc(sub)}</span><span class="bc mono">${count}</span>`;
  return b;
}

/* ---------- legend per category ---------- */
function legendHTML(cat) {
  const k = (b, l) => `<span class="k"><span class="kb">${b}</span><span class="kl">${l}</span></span>`;
  let items;
  if (cat === 'inbox') {
    items = k('→', 'to Next Actions') + k('→', 'to Delegated') + k('▢', 'Defer to Tickler') + k('✗', 'Drop');
  } else if (cat === 'delegated') {
    items = k('✓', 'Done') + k('↩', 'Back to me') + k('▢', 'Defer 1w/1m/1q') + k('✎', 'Edited');
  } else if (cat === 'tickler') {
    items = k('→', 'Activate now') + k('✓', 'Done') + k('▢', 'Re-defer') + k('✎', 'Edited');
  } else {
    items = k('✓', 'Done') + k('→', 'Delegate') + k('▢', 'Defer 1w/1m/1q') + k('✎', 'Edited');
  }
  ...
  return `
    <div class="head">
      <div class="head-l">
        ...
        <div>
          <div class="head-title">${meta.title}${contd ? ' <span style="font-weight:500;font-size:11pt">(cont\u2019d)</span>' : ''}</div>
...
function build() {
  ...
  // ----- INBOX -----
  {
    const meta = { tag: '0', title: 'Inbox', sub: 'Unprocessed capture — route every item out today', countLabel: `${DATA.inbox.length} to process` };
  ...
  // ----- NEXT ACTIONS -----
  {
    const meta = { tag: '1', title: 'Next Actions', sub: 'On your plate — do, delegate, or defer', countLabel: `${DATA.next.length} actions` };
    const blocks = DATA.next.map(t => rowEl('next', t));
    pages.push(buildPage('next', meta, blocks));
  }
  // ----- DELEGATED -----
  {
    const meta = { tag: '2', title: 'Delegated', sub: 'Waiting on others — follow up or reclaim', countLabel: `${DATA.delegated.length} waiting` };
    const blocks = DATA.delegated.map(t => rowEl('delegated', t));
    pages.push(buildPage('delegated', meta, blocks));
  }
  // ----- TICKLER -----
  {
    const meta = { tag: '3', title: 'Tickler', sub: 'Deferred — resurface when the time comes', countLabel: `${DATA.tickler.week.length + DATA.tickler.month.length + DATA.tickler.quarter.length} parked` };
    const blocks = [];
    const sub = [
...
window.addEventListener('load', () => { if (typeof qrcode !== 'undefined') build(); });
