...
- `assets/template.html.j2` — Jinja2 template with `data-roi` attributes on every interactive element. The `data-roi` values use canonical verbs (e.g. `NA-01:done`, `NA-01:defer_1w`, `CP-01:act`, `NA-01:new_project`, `NA-01:ai`, `P01-PJ:star`, `link:P01`, `link:P01@hot`). The ✦ AI box's verb is `ai`; sheets printed before the rename carry `edit`, which the scanner still reads as the AI flag (`decisions.AI_TICK_KEYS`).
...
- **Internal links**: `add_internal_links()` turns every `link:<ref>` ROI into a pypdf `Link` GoTo annotation after the render loop — summary block → project page, summary → `new-projects`, `← Projects` → summary, and the hotbar's `link:P01@hot` (anything after `@` only keeps the key unique on its page) → project page. ROI fractions are flipped into PDF user space against the page media box.
